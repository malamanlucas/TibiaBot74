...
void Player::killAllExpectfriend() {
	if (this->isAttacking()) {
		this->naviSender->comboTarget(ItemId::SD);
	}
	else {
		auto creature = v->battleList->findAllExpectFriend();
		if (creature != nullptr) {
			this->naviSender->comboTarget(ItemId::SD, creature->id());
		}
	}
}

bool Player::isVocationDruid() {
	return v->player->isDruid;
}

const int MIN_MANA_PARALYZE = 900;

bool Player::canShootParalyze() {
	return this->isVocationDruid() && this->mana() >= MIN_MANA_PARALYZE;
}
